...
def get_loaders(args, dataset, splits, directed):
    ...
    # do not use the subgraph, so use the general encoding
...
def get_data(args, device="cuda"):
    ...
    """
    # TODO: need to write for support more dataset here
    include_negatives = True
    dataset_name = args.dataset_name
    val_pct = args.val_pct # pencentage
    test_pct = args.test_pct
    use_lcc_flag = False # largest connected component
    # TODO: here the originasl setting is lcc for dataset except for ogb
    directed = False
    eval_metric = 'hits'
    ...
    if dataset_name.startswith('ogbl'):
        ...
        if dataset_name == 'ogbl-ddi':
            # DDI do not have any feature information
            dataset._data.x = torch.ones((dataset._data.num_nodes, 1))
            dataset._data.edge_weight = torch.ones(dataset._data.edge_index.size(1), dtype=int)
    else:
        dataset = Planetoid(path, dataset_name)
    # TODO: add more dataset here. 

    # set the metric
    if dataset_name.startswith('ogbl-citation'):
        eval_metric = 'mrr'
        directed = True
        # TODO: what is the direction here
    ...
    if dataset_name.startswith('ogbl'):  # use the built in splits
        ...
        # TODO: here we should have an alternative for the fixed split, and also for our split setting. 
    ...
    dataset._data = dataset._data.cuda()
...
def get_ogb_data(data, split_edge, dataset_name, num_negs=1):
    ...
    for key in split_edge.keys():
        ...
        # TODO: whether we ensure the that only folr the collab dataset
...
def use_lcc(dataset):
    # Use the largest connected component for training 
