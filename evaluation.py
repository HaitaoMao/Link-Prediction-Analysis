...
def evaluate_hits_single(evaluator, pos_pred, neg_pred, Ks=[20, 50, 100]):
    results = {}
    # As the training performance is used to assess overfitting it can help to use the same set of negs for
    # train and val comparisons.
    for K in Ks:
...
def evaluate_mrr_single(evaluator, pos_pred, neg_pred):
...
def eval_single(evaluator, pos_pred, neg_pred):
    mrr_results = evaluate_mrr_single(evaluator, pos_pred, neg_pred)
    hit_results = evaluate_hits_single(evaluator, pos_pred, neg_pred)

    results = {**mrr_results, **hit_results}
    return results

def eval_heart_single(args, pos_preds, neg_preds):    
    k_list = [20, 50, 100]
    result = {}

    result_mrr = evaluate_mrr(evaluator_mrr,  pos_preds, neg_preds)
   
    result['MRR'] = result_mrr['MRR']
    for K in k_list:
        result[f'Hits@{K}'] = result_mrr[f'mrr_hit{K}']
    return result
